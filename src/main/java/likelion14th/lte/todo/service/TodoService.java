...
public class TodoService {
    ...
    private record RoutineInfo(LocalDate startDate, LocalDate endDate, WeekEnum week) {}
    ...
    private RoutineInfo resolveRoutineInfo(boolean routineEnabled, LocalDate startDate, LocalDate endDate, WeekEnum week) {
        if (!routineEnabled) return new RoutineInfo(null, null, null);
        validateRoutineRequest(startDate, endDate, week);
        return new RoutineInfo(startDate, endDate, week);
    }
    ...
    public TodoDetailResponse createTodo(Long userId, TodoCreateRequest request, LocalDate date) {
        ...
        RoutineInfo ri = resolveRoutineInfo(routineEnabled,
                request.getStartDate(), request.getEndDate(), request.getWeek());

        Todo todo = Todo.create(user, request.getDescription(), routineEnabled,
                ri.startDate(), ri.endDate(), category, ri.week());
        todoRepository.save(todo);

        if (routineEnabled) {
            routineTodoDateGenerator.generate(todo, ri.startDate(), ri.endDate(), ri.startDate());
        } else {
    ...
    public TodoDetailResponse updateTodoDetail(Long userId, Long todoId, TodoUpdateRequest request) {
        ...
        todo.update(request.getDescription(), category, ri.week(), routineEnabled, ri.startDate(), ri.endDate());

        if (routineEnabled) {
            LocalDate today = LocalDate.now();
            todoDateRepository.deleteAllByTodo_IdAndDateGreaterThanEqual(todoId, today);
            routineTodoDateGenerator.generate(todo, ri.startDate(), ri.endDate(), today);
        }
